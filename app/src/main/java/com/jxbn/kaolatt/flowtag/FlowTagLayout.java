package com.jxbn.kaolatt.flowtag;
...
//              new MarginLayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT));//这个构造方法所然能使用但是编译器会报错
            ...
            if (mAdapter instanceof OnInitSelectedPosition) {
                boolean isSelected = ((OnInitSelectedPosition) mAdapter).isSelectedPosition(i);//拿到每个条目的状态
                //判断一下模式
            ...
            childView.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    if (mTagCheckMode == FLOW_TAG_CHECKED_NONE) {
                        ...
                        //判断状态
                        if (mCheckedTagArray.get(j)) {//已选中的Tag
                    ...
                    } else if (mTagCheckMode == FLOW_TAG_CHECKED_MULTI) {//多选模式
                        if (mCheckedTagArray.get(j)) {//原来的状态为选中
                            mCheckedTagArray.put(j, false);//改为未选中
                            childView.setSelected(false);//改为未选中
                        } else {
                            mCheckedTagArray.put(j, true);
                            childView.setSelected(true);
                        }
